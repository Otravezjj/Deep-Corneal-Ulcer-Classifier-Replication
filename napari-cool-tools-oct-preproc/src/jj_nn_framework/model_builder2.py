...
class SC_ConvBlock(nn.Module):
    def __init__(self, input_chs, num_filters, kernel_size, padding=2, padding_mode='zeros'):
        super().__init__()
        self.relu  = nn.ReLU()
        self.batch_n_2d = nn.BatchNorm2d(num_filters)
        self.conv2d = nn.Conv2d(
            input_chs, 
            num_filters, 
            kernel_size,
            padding=padding,
            padding_mode=padding_mode,bias=False) #bias optimization
...
class SC_Length(nn.Module):
    ...
    def forward(self, x, **kwargs):
        if x.ndim == 5:
            ...
            x = x.squeeze(dim=-2)
            x = torch.norm(x,dim=-1).unsqueeze(dim=-1)
        if self.seg:
...
class SC_ConvCapsuleLayer(nn.Module):
    ...
    def build(self):
        ...
        # weights are (out_channels/atoms,input_channels/atoms,kH,kW)
    ...
    def forward(self,x):
        ...
        x = x.view(-1,self.input_height,self.input_width,self.input_num_capsule,self.input_num_atoms)
        ...
        conv = F.conv2d(input_tensor_reshaped,self.W,stride=(self.strides,self.strides),padding=self.padding,groups=1) # had
        # to add groups setting inorder for weights to work properly
