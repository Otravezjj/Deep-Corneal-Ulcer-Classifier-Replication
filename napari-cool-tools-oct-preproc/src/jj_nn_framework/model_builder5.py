...
from jj_nn_framework.vin_ai_layers2 import ConvSlimCapsule2D
...
class BasicSegCaps(nn.Module):
    def __init__(self, params_list):
        ...
        self.PrimaryCaps = ConvSlimCapsule2D(**params_list[1])
        self.SegCaps = ConvSlimCapsule2D(**params_list[2])
...
class SC_ConvBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        # input N,C,H,W # C = Atoms -> A, Ct = capsule type
        x = self.conv2d(x)
        N,C,H,W = x.shape
        x = self.relu(self.batch_n_2d(x)) # added batch normalization
        reshape = (-1,H,W,C)
        x = x.view(reshape) # N,H,W,A
        x = x.unsqueeze(3) # N,H,W,Ct,A
        return x.permute(0,3,4,1,2) # change for CapsuleSlim2D
...
class SC_Length(nn.Module):
    ...
    def forward(self, x, **kwargs):
        # input N,Ct,A,H,W
        if x.ndim == 5:
            assert x.shape[1] == 1, 'Error: Must have num_capsules = 1 going into Length'
            x = x.squeeze(dim=1)
        ...
        if self.seg:
            ...
            #x = x @ classes # Argh!!
            #x = x.permute(0,3,1,2)
            x = x.expand((-1,-1,-1,self.num_classes))
            x = x.permute(0,3,1,2)
        else:
            pass
...
class SC_ConvCapsuleLayer(nn.Module):
    ...
    def build(self):
        ...
        self.weights = nn.Conv2d(
                self.input_num_atoms, self.num_capsule * self.num_atoms, self.kernel_size, stride=self.strides, padding=self.padding,device=self.device
            )
    ...
    def forward(self,x):
        ...
        conv = self.weights(input_tensor_reshaped)
