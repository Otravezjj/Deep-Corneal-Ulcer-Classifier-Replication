...
class v2_CardinalPermutations(v2.Transform):
    _transformed_types: tuple[type | Callable[[Any], bool], ...] = (Image.Image, tv_tensors.Image, torch.Tensor, tv_tensors.Mask, str)
    def __init__(self):
        super().__init__()
    ...
    def _transform(self, inpt:Any, params: Dict[str,Any]):
        if isinstance(inpt,tv_tensors.Image) or isinstance(inpt, Image.Image):
            ...
            if inpt.ndim < 3:
                raise ValueError(f"Input has {inpt.ndim} dimensions, 3 or 4 dimensions are required")
            if inpt.ndim < 4:
                inpt.unsqueeze()
            ...
            images.append(v2.functional.horizontal_flip(v2.functional.vertical_flip(inpt)))
...
class v2_ImageCrops(v2.Transform):
    ...
    def _transform(self, inpt:Any, params: Dict[str,Any]):
        ...
        if isinstance(inpt,tv_tensors.Image) or isinstance(inpt, Image.Image):
            ...
            crops_inpt = v2_get_crops(inpt,kh,kw,h_indicies,w_indicies,shuffle=self.shuffle,device=self.device,verbose=self.verbose)
            
            if self.verbose:
                print(f"num_crops = {len(crops_inpt)}, shapes per crop x,y: {crops_inpt[0].shape},{crops_inpt[0].shape}\n")
